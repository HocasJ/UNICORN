...
class Unlabeld_SYSUData_Pseudo(data.Dataset):
    def __init__(self,args,data_dir,transform,rgb_cluster=False,ir_cluster=False):
        self.train_color_image = np.load(data_dir+args.exp_setting+'train_rgb_resized_img.npy')
        self.train_thermal_image = np.load(data_dir+args.exp_setting+'train_ir_resized_img.npy')

        self.train_color_label = np.load(data_dir + args.exp_setting+'train_rgb_resized_label.npy')
        self.train_thermal_label = np.load(data_dir + args.exp_setting+'train_ir_resized_label.npy')

        self.train_color_path = np.load(data_dir + args.exp_setting+'train_rgb_resized_path.npy')
        self.train_thermal_path = np.load(data_dir + args.exp_setting+'train_ir_resized_path.npy')
    ...
    def __len__(self):
        if self.ir_cluster:
            ...
            return len(self.train_color_path)
        else:
            print("error len!!")


class Unlabeld_SYSUData(data.Dataset):
    def __init__(self,args, transform,rgb_cluster=False,ir_cluster=False):
        self.train_color_image = np.load(args.data_path + 'train_rgb_resized_img.npy')
        self.train_thermal_image = np.load(args.data_path + 'train_ir_resized_img.npy')

        self.train_color_label = np.load(args.data_path + 'train_rgb_resized_label.npy')
        self.train_thermal_label = np.load(args.data_path + 'train_ir_resized_label.npy')

        self.train_color_path = np.load(args.data_path + 'train_rgb_resized_path.npy')
        self.train_thermal_path = np.load(args.data_path + 'train_ir_resized_path.npy')
...
class Unlabeld_MMMPData(data.Dataset):
    def __init__(self,args, transform,rgb_cluster=False,ir_cluster=False):
        self.train_color_image = np.load(args.data_path + args.exp_setting+'train_rgb_resized_img.npy')
        self.train_thermal_image = np.load(args.data_path + args.exp_setting+'train_ir_resized_img.npy')

        self.train_color_label = np.load(args.data_path + args.exp_setting+'train_rgb_resized_label.npy')
        self.train_thermal_label = np.load(args.data_path + args.exp_setting+'train_ir_resized_label.npy')

        self.train_color_path = np.load(args.data_path + args.exp_setting+'train_rgb_resized_path.npy')
        self.train_thermal_path = np.load(args.data_path + args.exp_setting+'train_ir_resized_path.npy')
...
class SYSUData_Stage2(data.Dataset):
    def __init__(self, args, pseudo_labels_rgb=None, pseudo_labels_ir=None,transform_train_rgb=None, transform_train_ir=None, colorIndex=None, thermalIndex=None):
        # Load training images (path) and labels
    ...
    def __getitem__(self, index):
        img1, target1, target1_new = self.train_color_image[self.cIndex[index]], self.train_color_label[self.cIndex[index]], self.train_color_pseudo_label[self.cIndex[index]]
        img2, target2, target2_new = self.train_thermal_image[self.tIndex[index]],  self.train_thermal_label[self.tIndex[index]], self.train_thermal_pseudo_label[self.tIndex[index]]
        img1 = self.transform_train_rgb(img1)
        img2 = self.transform_train_ir(img2)

        return img1, img2, target1_new, target2_new, target1, target2

    def __len__(self):
        return len(self.train_color_label)


class MMMPData_Stage2(data.Dataset):
    def __init__(self, args, pseudo_labels_rgb=None, pseudo_labels_ir=None,transform_train_rgb=None, transform_train_ir=None, colorIndex=None, thermalIndex=None):
        # Load training images (path) and labels
...
class SYSUData(data.Dataset):
    def __init__(self, data_dir,  transform=None, colorIndex = None, thermalIndex = None, rgb = None, ir = None):
        ...
        self.cIndex = colorIndex
        self.tIndex = thermalIndex
        self.num_classes_color = len(self.train_color_label)
        self.num_classes_thermal = len(self.train_thermal_label)
    ...
    def __getitem__(self, index):
        ...
        if self.rgb:
            img1,  target1 = self.train_color_image[self.cIndex[index]],  self.train_color_label[self.cIndex[index]]
            img1 = self.transform(img1)
            return img1, target1, "RGB"
        elif self.ir:
            img2,  target2 = self.train_thermal_image[self.tIndex[index]], self.train_thermal_label[self.tIndex[index]]
            img2 = self.transform(img2)
            return img2, target2,  'IR'
        else:
            print('error getitem!')
