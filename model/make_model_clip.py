...
class visible_module(nn.Module):
    ...
    def forward(self, x):
        ...
        x1 = self.visible.conv1(x)
        x1 = self.visible.bn1(x1)
        x1 = self.visible.relu(x1)

        x2 = self.visible.conv2(x1)
        x2 = self.visible.bn2(x2)
        x2 = self.visible.relu(x2)

        x3 = self.visible.conv3(x2)
        x3 = self.visible.bn3(x3)
        x3 = self.visible.relu(x3)

        x4 = self.visible.avgpool(x3)
        # x = self.pooling(x)
        return x4
...
class base_resnet(nn.Module):
    def __init__(self, model_name, h_resolution, w_resolution, vision_stride_size):
        ...
        self.attnpool_1 = AttentionPool2d(72*36, 256, 8, 2048)
        self.attnpool_2 = AttentionPool2d(36*18, 512, 8, 2048)
        self.attnpool_3 = AttentionPool2d(18*9, 1024, 8, 2048)
    ...
    def forward(self, x):
        ...
        x1_attn = self.attnpool_1(x1)
        x2_attn = self.attnpool_2(x2)
        x3_attn = self.attnpool_3(x3)
        return x1_attn, x2_attn, x3_attn, x4
...
class build_model(nn.Module):
    def __init__(self, args, num_classes_rgb, num_classes_ir):
        ...
        self.avgpool_feat = nn.AdaptiveAvgPool2d((pool_dim, 1))
    ...
    def forward(self, x1=None, x2=None, modal=0, get_image=False, get_text=False, label=None):
        ...
        x1_attn, x2_attn, x3_attn, x = self.base_resnet(x)
        image_features_proj = self.base_resnet.base.attnpool(x)
        ...
        new_feat = self.avgpool_feat(torch.cat((feat.unsqueeze(2), x1_attn[0].unsqueeze(2)), dim=2))
        
        new_feat = new_feat.squeeze(2)
        if self.training:            
            return new_feat, image_features_proj[0]
        else:
            return self.l2norm(new_feat)
            # return feat
