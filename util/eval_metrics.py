...
def extract_cnn_feature(model, inputs, modal):
    inputs = inputs.cuda()
    outputs = model(inputs, inputs, modal=modal)
    return outputs

def extract_features(model, data_loader, modal):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for i, (imgs, pids, paths, _) in loop:
            ...
            for path, output, pid in zip(paths, outputs, pids):
...
def extract_features_clip(model, data_loader, modal, get_image=False):
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    with torch.no_grad():
        ...
        for i, (imgs, pids, paths, _) in loop:
            ...
            # if get_image==False: breakpoint()
            for path, output, pid in zip(paths, outputs, pids):
                features[path] = output.cpu()
                # if features[path].shape[0] != 64:
                #     print(i)
                labels[path] = pid
